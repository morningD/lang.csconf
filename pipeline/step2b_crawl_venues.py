...
def run(force: bool = False, conferences_filter: list[str] | None = None):
    ...
    for conf in conferences:
        ...
        # For multi-key conferences, fetch all keys and merge venues
        dblp_keys = dblp_value if isinstance(dblp_value, list) else [dblp_value]
        to_fetch.append((conf_id, dblp_keys))
    ...
    for conf_id, dblp_keys in tqdm(to_fetch, desc="Venue crawl"):
        try:
            # Merge venues from all DBLP keys (for multi-stream conferences)
            venues = {}
            for dblp_key in dblp_keys:
                key_venues = _fetch_venues_from_html(dblp_key)
                for year, info in key_venues.items():
            ...
            result = {
                "conference": conf_id,
                "dblp": dblp_keys[0] if len(dblp_keys) == 1 else dblp_keys,
                "venues": venues,
            }
            ...
            print(f"\n  Error fetching {conf_id} ({dblp_keys}): {e}")
            errors += 1
